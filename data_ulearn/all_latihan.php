<?php
    ...
    $id_user = $_GET['idUser'];
    $query = $DBH->query("
        SELECT p.`id_dosen` AS id_dosen, p.`title`, p.`intro`, p.`isi`, l.`koreksi`, p.`create_date`, p.`id_posting`, p.`latihan`,l.`read_by_user`, l.`id_latihan`, l.`jawaban`,  l.`id_mahasiswa` as id_mahasiswa
        FROM posting AS p 
